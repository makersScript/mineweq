...
# ─── SKIN GENERATOR: ЧАСТИ ТЕЛА И UV-РАЗМЕТКА ──────────────────────────────────
# Формат обычного скина 64x64 общий и для Java, и для Bedrock (классическая
# модель) — поэтому одна и та же картинка подходит под обе версии, разница
# только в том, как мы её потом упаковываем и отдаём пользователю.
SKIN_PARTS = [
    ("head",      "🗣 Голова"),
    ("body",      "🫀 Тело (торс)"),
...
SKIN_PART_LABELS = dict(SKIN_PARTS)

# region: (x, y, w, h) — координаты внутри итоговой PNG 64x64
SKIN_REGIONS = {
    "head": {
        "front": (8, 8, 8, 8), "back": (24, 8, 8, 8),
        "left":  (16, 8, 8, 8), "right": (0, 8, 8, 8),
        "top":   (8, 0, 8, 8),  "bottom": (16, 0, 8, 8),
    },
    "body": {
        "front": (20, 20, 8, 12), "back": (32, 20, 8, 12),
        "left":  (28, 20, 4, 12), "right": (16, 20, 4, 12),
        "top":   (20, 16, 8, 4),  "bottom": (28, 16, 8, 4),
    },
    "arm_right": {
        "front": (44, 20, 4, 12), "back": (52, 20, 4, 12),
        "left":  (48, 20, 4, 12), "right": (40, 20, 4, 12),
        "top":   (44, 16, 4, 4),  "bottom": (48, 16, 4, 4),
    },
    "arm_left": {
        "front": (36, 52, 4, 12), "back": (44, 52, 4, 12),
        "left":  (40, 52, 4, 12), "right": (32, 52, 4, 12),
        "top":   (36, 48, 4, 4),  "bottom": (40, 48, 4, 4),
    },
    "leg_right": {
        "front": (4, 20, 4, 12), "back": (12, 20, 4, 12),
        "left":  (8, 20, 4, 12), "right": (0, 20, 4, 12),
        "top":   (4, 16, 4, 4),  "bottom": (8, 16, 4, 4),
    },
    "leg_left": {
        "front": (20, 52, 4, 12), "back": (28, 52, 4, 12),
        "left":  (24, 52, 4, 12), "right": (16, 52, 4, 12),
        "top":   (20, 48, 4, 4),  "bottom": (24, 48, 4, 4),
    },
}

SKIN_PROMPTS = {
    "head":      "Пришли фото <b>лица</b> — квадратное фото ляжет лучше всего.",
    "body":      "Пришли фото <b>торса/одежды</b> (перед, крупным планом).",
    "arm_right": "Пришли фото <b>правой руки</b> (кожа или рукав).",
    "arm_left":  "Пришли фото <b>левой руки</b>.",
    "leg_right": "Пришли фото <b>правой ноги</b> (штаны/обувь).",
    "leg_left":  "Пришли фото <b>левой ноги</b>.",
}

# ─── SKIN GENERATOR: СБОРКА КАРТИНКИ ───────────────────────────────────────────
def _fit_region(photo_bytes: bytes, w: int, h: int) -> Image.Image:
...
def _darken(img: Image.Image, factor: float = 0.85) -> Image.Image:
    r, g, b, a = img.split()
...
def build_skin_png(part_photos: dict) -> bytes:
    """part_photos: {part_key: photo_bytes}. Собирает готовый skin.png 64x64.
    Лицевая грань — реальное фото, задняя — то же фото затемнённое,
    остальные грани (бока/верх/низ) — заливка средним цветом с фото."""
    canvas = Image.new("RGBA", (64, 64), (0, 0, 0, 0))
    for part_key, photo_bytes in part_photos.items():
        regions = SKIN_REGIONS[part_key]
        fx, fy, fw, fh = regions["front"]
        front_img = _fit_region(photo_bytes, fw, fh)
        canvas.paste(front_img, (fx, fy), front_img)
        avg = _avg_color(front_img)
        fill_cache = {}
        for face, (x, y, w, h) in regions.items():
            if face == "front":
                continue
            if face == "back":
                back_img = front_img.resize((w, h), Image.LANCZOS)
                canvas.paste(_darken(back_img), (x, y), back_img)
                continue
            key = (w, h)
            if key not in fill_cache:
                fill_cache[key] = Image.new("RGBA", (w, h), avg)
            canvas.paste(fill_cache[key], (x, y))
    buf = io.BytesIO()
    canvas.save(buf, format="PNG")
    return buf.getvalue()

def build_bedrock_skin_pack(skin_png: bytes, pack_name: str) -> bytes:
    """Упаковывает skin.png в .mcpack (skin pack) для Bedrock Edition."""
    buf = io.BytesIO()
    manifest = {
        "format_version": 2,
        "header": {
            "name": pack_name,
            "description": f"{pack_name} — создано в PackCraftBot",
            "uuid": str(uuid.uuid4()),
            "version": [1, 0, 0],
            "min_engine_version": [1, 16, 0],
        },
        "modules": [
            {"type": "skin_pack", "uuid": str(uuid.uuid4()), "version": [1, 0, 0]}
        ],
    }
    skins_json = {
    ...
    lang = f"skinpack.{pack_name}={pack_name}\nskin.{pack_name}.skin1={pack_name}\n"
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        zf.writestr("manifest.json", json.dumps(manifest, indent=2, ensure_ascii=False))
        zf.writestr("skins.json", json.dumps(skins_json, indent=2, ensure_ascii=False))
        zf.writestr("skin1.png", skin_png)
        zf.writestr("texts/en_US.lang", lang)
    return buf.getvalue()
...
class AdminStates(StatesGroup):
    give_sub_id       = State()
    give_sub_type     = State()
    broadcast_wait    = State()   # ждём контент для рассылки
    broadcast_button_choice = State()  # добавлять кнопку или нет
    broadcast_button_text   = State()  # ждём текст кнопки
    broadcast_button_url    = State()  # ждём ссылку кнопки
    broadcast_confirm = State()  # ждём подтверждения перед отправкой всем
...
def main_menu_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🧑‍🎨 Скин из фото 💎",   callback_data="create_skin")],
    ])
...
def skin_version_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="☕ Java Edition",   callback_data="skinver_java")],
        [InlineKeyboardButton(text="📱 Bedrock Edition", callback_data="skinver_bedrock")],
        [InlineKeyboardButton(text="⬅️ Назад",          callback_data="main_menu")],
    ])
...
def broadcast_button_choice_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="➕ Добавить кнопку со ссылкой", callback_data="admin_broadcast_add_button")],
        [InlineKeyboardButton(text="➡️ Без кнопки", callback_data="admin_broadcast_skip_button")],
        [InlineKeyboardButton(text="❌ Отмена", callback_data="admin_broadcast_cancel")],
    ])

def broadcast_confirm_kb(count: int):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"✅ Отправить всем ({count})", callback_data="admin_broadcast_send")],
        [InlineKeyboardButton(text="❌ Отмена", callback_data="admin_broadcast_cancel")],
    ])
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if not await check_channel_subscription(message.from_user.id):
        ...
        return
    await message.answer(
        "🎮 <b>PackCraftBot</b> — инструмент, который помогает в Minecraft: "
        "создавай свои ресурс-паки и скины прямо в Telegram, без программ на компьютере.\n\n"
        "• 🖼 <b>Текстуры</b> — блоки, мобы, броня, инструменты, GUI\n"
        "• 🔊 <b>Звуки</b> — замени любой звук игры\n"
        "• 🧑‍🎨 <b>Скины</b> — собери скин из своих фото\n"
        "• ☕ Java Edition и 📱 Bedrock Edition\n\n"
        "🆓 <b>Бесплатно:</b> 1 пак\n"
        "💎 <b>Подписка:</b> безлимитные паки и скины\n\n"
        "Выбери действие:",
...
# ─── SKIN GENERATOR (фото → скин, только по подписке, без бесплатной пробы) ────
@dp.callback_query(F.data == "create_skin")
async def cb_create_skin(cq: CallbackQuery, state: FSMContext):
    uid = cq.from_user.id
    if not is_subscribed(uid):
        await cq.message.edit_text(
            "💎 <b>Генератор скинов из фото — только по подписке</b>\n\n"
            "Бот соберёт скин из твоих фото по частям тела: голова, тело, руки, ноги.\n"
            "У этой функции нет бесплатной пробы — нужна активная подписка.",
            reply_markup=skin_paywall_kb(), parse_mode="HTML"
        )
        return
    await state.clear()
    await state.set_state(SkinStates.choose_version)
    await cq.message.edit_text(
        "🌍 <b>Для какой версии Minecraft собрать скин?</b>\n\n"
        "☕ Java — получишь <code>skin.png</code>, который сам загрузишь на minecraft.net\n"
        "📱 Bedrock — получишь готовый <code>.mcpack</code>, который сам установится",
        reply_markup=skin_version_kb(), parse_mode="HTML"
    )

@dp.callback_query(SkinStates.choose_version, F.data.startswith("skinver_"))
async def cb_skin_version(cq: CallbackQuery, state: FSMContext):
    version = cq.data.split("_", 1)[1]
    await state.update_data(skin_version=version, skin_photos={}, skin_index=0)
    await state.set_state(SkinStates.upload_part)
    await cq.message.edit_text(
        "📸 <b>Сейчас поочерёдно пришли фото для каждой части тела.</b>\n"
        "Можно как фото, так и файлом PNG/JPG.",
        parse_mode="HTML"
    )
    await _ask_next_skin_part(cq.message, state)

async def _ask_next_skin_part(message: Message, state: FSMContext):
    data = await state.get_data()
    idx = data.get("skin_index", 0)
    if idx >= len(SKIN_PARTS):
        await _finish_skin(message, state)
        return
    key, label = SKIN_PARTS[idx]
    await message.answer(
        f"<b>Шаг {idx + 1}/{len(SKIN_PARTS)}: {label}</b>\n\n{SKIN_PROMPTS[key]}",
        parse_mode="HTML"
    )

@dp.message(SkinStates.upload_part, F.photo | F.document)
async def upload_skin_part(message: Message, state: FSMContext):
    if message.photo:
        file = await bot.get_file(message.photo[-1].file_id)
    elif message.document:
        fname = (message.document.file_name or "").lower()
        if not (fname.endswith(".png") or fname.endswith(".jpg") or fname.endswith(".jpeg")):
            await message.answer("❌ Отправь фото в виде PNG/JPG.")
            return
        file = await bot.get_file(message.document.file_id)
    else:
        return
    ...
    data = await state.get_data()
    idx = data.get("skin_index", 0)
    key, _ = SKIN_PARTS[idx]
    photos = data.get("skin_photos", {})
    photos[key] = buf.getvalue()
    await state.update_data(skin_photos=photos, skin_index=idx + 1)
    await _ask_next_skin_part(message, state)

@dp.message(SkinStates.upload_part)
async def upload_skin_part_wrong(message: Message, state: FSMContext):
    await message.answer("❌ Пришли именно фото — как изображение или файлом PNG/JPG.")

async def _finish_skin(message: Message, state: FSMContext):
    data = await state.get_data()
    version = data.get("skin_version", "java")
    photos  = data.get("skin_photos", {})
    uid     = message.chat.id

    status = await message.answer("⏳ Собираю скин из твоих фото...")

    try:
        skin_png = build_skin_png(photos)
        if version == "java":
            file_bytes = skin_png
            filename = "skin.png"
            caption = (
                "✅ <b>Скин готов!</b>\n\n"
                "Это Java-скин. Загрузи <code>skin.png</code> в свой профиль на "
                "<b>minecraft.net</b>, чтобы он применился в игре."
            )
        else:
            pack_name = f"PhotoSkin_{uid}"
            file_bytes = build_bedrock_skin_pack(skin_png, pack_name)
            filename = f"{pack_name}.mcpack"
            caption = (
                "✅ <b>Скин готов!</b>\n\n"
                "Это Bedrock skin pack. Открой файл на телефоне — он установится "
                "в Minecraft автоматически, дальше выбери его в разделе персонажа."
            )
    except Exception as e:
        await state.clear()
        await status.edit_text(
            f"❌ <b>Ошибка сборки скина:</b> <code>{e}</code>\n\nПопробуй ещё раз.",
        )
...
# ─── АДМИН: РАССЫЛКА (текст/фото/видео/файл — что угодно, всем пользователям) ──
...
async def cb_admin_broadcast(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        "📣 <b>Рассылка</b>\n\n"
        "Пришли сюда сообщение, которое разослать всем пользователям бота — "
        "текст, фото, видео, документ, голосовое, стикер и т.д. Отправится "
        "как есть, вместе с подписью.",
        reply_markup=broadcast_wait_kb(), parse_mode="HTML"
    )

@dp.callback_query(F.data == "admin_broadcast_cancel")
async def cb_admin_broadcast_cancel(cq: CallbackQuery, state: FSMContext):
    if cq.from_user.id not in ADMIN_IDS:
        return
    await state.clear()
    await cq.message.edit_text("❌ Рассылка отменена.", reply_markup=admin_kb())
...
async def admin_broadcast_receive(message: Message, state: FSMContext):
    ...
    await message.answer(
        "👆 <b>Вот так будет выглядеть рассылка.</b>\n\n"
        "Добавить под сообщением кнопку со ссылкой (например, на канал или сайт)?",
        reply_markup=broadcast_button_choice_kb(), parse_mode="HTML"
    )

@dp.callback_query(AdminStates.broadcast_button_choice, F.data == "admin_broadcast_skip_button")
async def cb_broadcast_skip_button(cq: CallbackQuery, state: FSMContext):
    if cq.from_user.id not in ADMIN_IDS:
        return
    await state.update_data(broadcast_button_text=None, broadcast_button_url=None)
    await _ask_broadcast_confirm(cq.message, state)

@dp.callback_query(AdminStates.broadcast_button_choice, F.data == "admin_broadcast_add_button")
async def cb_broadcast_add_button(cq: CallbackQuery, state: FSMContext):
    if cq.from_user.id not in ADMIN_IDS:
        return
    await state.set_state(AdminStates.broadcast_button_text)
    await cq.message.edit_text(
        "✏️ <b>Введи текст кнопки</b>\n\nНапример: <i>Подписаться на канал</i>",
        parse_mode="HTML"
    )

@dp.message(AdminStates.broadcast_button_text)
async def admin_broadcast_button_text(message: Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS:
        return
    text = (message.text or "").strip()[:64]
    if not text:
        await message.answer("❌ Пришли текстом название кнопки.")
        return
    await state.update_data(broadcast_button_text=text)
    await state.set_state(AdminStates.broadcast_button_url)
    await message.answer(
        "🔗 <b>Теперь пришли ссылку для кнопки</b>\n\nНапример: <code>https://t.me/mychannel</code>",
        parse_mode="HTML"
    )

@dp.message(AdminStates.broadcast_button_url)
async def admin_broadcast_button_url(message: Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS:
        return
    url = (message.text or "").strip()
    if not (url.startswith("http://") or url.startswith("https://") or url.startswith("tg://")):
        await message.answer("❌ Ссылка должна начинаться с <code>https://</code>. Пришли ещё раз.",
                              parse_mode="HTML")
        return
    await state.update_data(broadcast_button_url=url)
    await _ask_broadcast_confirm(message, state)

async def _ask_broadcast_confirm(message: Message, state: FSMContext):
    data  = await state.get_data()
    ...
    await message.answer(
        f"Отправить всем пользователям бота (<b>{count}</b>)?",
        reply_markup=broadcast_confirm_kb(count), parse_mode="HTML"
    )

@dp.callback_query(AdminStates.broadcast_confirm, F.data == "admin_broadcast_send")
async def cb_admin_broadcast_send(cq: CallbackQuery, state: FSMContext):
    if cq.from_user.id not in ADMIN_IDS:
        return
    data = await state.get_data()
    ...
    if not src_chat_id or not src_msg_id:
        await cq.message.edit_text("❌ Сообщение для рассылки потерялось, попробуй ещё раз.",
    ...
    await cq.message.edit_text("⏳ <b>Рассылка запущена...</b>", parse_mode="HTML")
    ...
    for uid in user_ids:
        try:
            ...
            failed += 1
        await asyncio.sleep(0.05)  # анти-флуд, чтобы не упереться в лимиты Telegram

    await bot.send_message(
        cq.from_user.id,
        f"✅ <b>Рассылка завершена</b>\n\n"
        f"📬 Доставлено: <b>{sent}</b>\n"
        f"🚫 Не доставлено (бот заблокирован и т.п.): <b>{failed}</b>",
